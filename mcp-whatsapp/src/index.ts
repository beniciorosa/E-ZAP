...
let whatsapp: InstanceType<typeof Client>;
...
function setupClientEvents(client: InstanceType<typeof Client>): void {
...
function ensureReady(): void {
  if (!whatsapp) {
    throw new Error(
      "WhatsApp client not initialized. Use whatsapp_reconnect to start."
    );
  }
  if (!clientReady) {
    throw new Error(
      `WhatsApp not ready yet. Status: ${clientStatus}. ` +
        (clientStatus === "authenticated"
...
server.tool(
  "whatsapp_reconnect",
  "Reconnect WhatsApp client. Use force=true to force reconnection even if status shows connected.",
  {
...
server.tool(
  "whatsapp_get_qr",
  "Get QR code string for WhatsApp Web authentication. The QR code is also printed in the server terminal.",
  {},
  async () => {
    if (clientReady) {
      return {
...
server.tool(
  ...
  "Send a text message to a WhatsApp contact or group",
  {
    to: z
      .string()
      .describe(
        "Phone number with country code (e.g. 5511999999999) or group ID"
      ),
...
server.tool(
  "whatsapp_send_image",
  "Send an image with optional caption to a WhatsApp contact or group",
  {
...
server.tool(
  ...
  "Read recent messages from a specific chat",
  {
    chatId: z
      .string()
      .describe(
        "Phone number with country code (e.g. 5511999999999) or group ID (e.g. 120363xxxxx@g.us)"
      ),
    limit: z
      .number()
      .optional()
...
server.tool(
  ...
  async ({ query, chatId, limit }) => {
    ...
    const formatted = messages.slice(0, limit).map((msg: any) => ({
      id: msg.id.id,
      ...
      content: [
        {
          type: "text",
          text: JSON.stringify(
            { query, resultCount: formatted.length, results: formatted },
            null,
            2
          ),
        },
...
server.tool(
  "whatsapp_get_chats",
  "List all WhatsApp chats (conversations)",
  {
  ...
  async ({ limit, unreadOnly }) => {
    ...
    if (unreadOnly) {
      chats = chats.filter((c: any) => c.unreadCount > 0);
    }

    const formatted = chats.slice(0, limit).map((chat: any) => ({
      ...
      content: [
        {
          type: "text",
          text: JSON.stringify(
            { totalChats: chats.length, chats: formatted },
            null,
            2
          ),
        },
...
server.tool(
  ...
  async ({ limit, search }) => {
    ...
    contacts = contacts.filter((c: any) => c.isUser && c.id._serialized !== "status@broadcast");
    ...
    const formatted = contacts.slice(0, limit).map((c: any) => ({
      id: c.id._serialized,
      name: c.name || null,
      ...
      content: [
        {
          type: "text",
          text: JSON.stringify(
            { totalContacts: contacts.length, contacts: formatted },
            null,
            2
          ),
        },
...
server.tool(
  ...
  async ({ limit }) => {
    ...
    const formatted = groups.slice(0, limit).map((g: any) => ({
      id: g.id._serialized,
      name: g.name,
      ...
      content: [
        {
          type: "text",
          text: JSON.stringify(
            { totalGroups: groups.length, groups: formatted },
            null,
            2
          ),
        },
...
server.tool(
  ...
  "Change the name/subject of a WhatsApp group (requires admin)",
  {
  ...
  async ({ groupId, subject }) => {
    ensureReady();
    const chat = await whatsapp.getChatById(groupId);
    if (!(chat as any).isGroup) throw new Error("Not a group chat");
...
server.tool(
  ...
  "Change the description of a WhatsApp group (requires admin)",
  {
    groupId: z.string().describe("Group ID"),
...
server.tool(
  "whatsapp_get_labels",
  "List all WhatsApp labels/tags (WhatsApp Business feature)",
  {},
  async () => {
    ensureReady();
    try {
      const labels = await whatsapp.getLabels();
      const formatted = labels.map((l: any) => ({
        id: l.id,
        name: l.name,
        color: l.hexColor,
        count: l.count || 0,
...
server.tool(
  ...
  "Add a label to a chat (WhatsApp Business feature)",
  {
